...
- [x] Attachment preview with image and text/data file payload support.
...
- [x] Drag-over visual state for attachment upload.
  - Legacy adds a visual drag state when files are dragged over the composer.
  - Vue accepts file picker, drag/drop, and clipboard paste attachments.
...
- [x] Attachment menu parity.
  ...
  - The UI and API use attachment naming, while the active provider path supports up to 4 images or text/data files per message.
...
- Upload, drop, or paste one or more supported attachments.
